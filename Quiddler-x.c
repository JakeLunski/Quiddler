...
// Get the point value of a single or two-letter card based on the Quiddler scoring chart.
int getLetterPoints(const char *str) {
    // Get the length of the input string (1 or 2 characters)
    int length = strlen(str);

    // If the input string has 2 characters, check if it matches any of the two-letter card combinations
    if (length == 2) {
        if (strcmp(str, "er") == 0) {
            return 7;
        }
    ...
    // If the input has only 1 character, get the point value for the single-letter card
    if (length == 1) {
        switch (tolower(str[0])) {      // Convert the character to lowercase to handle both uppercase and lowercase inputs
            // For each case, return the corresponding point value of the single-letter card
            case 'a': return 2;
            case 'b': return 8;
            case 'c': return 8;
            case 'd': return 5;
            ...
            case 'y': return 4;
            case 'z': return 14;
            default: return 0;      // If the input character is not a valid card, return 0 points
        }
    }

    return 0;   // If the input string is not 1 or 2 characters, return 0 points
}

//  Calculate the total points of a word formed using the cards.
int wordPoints(char *word, char **cards, int numCards, int wordLength) {
    // Initialize the total points variable
    int points = 0;
    // Create a temporary array for storing 1 or 2 character combinations
    char temp[3] = {0};

    // Iterate through the input word
    for (int i = 0; i < wordLength; i++) {
        if (i < wordLength - 1) {       // Check if we have at least two characters left in the word
            temp[0] = word[i];          // Assign the current and next characters to the temp array
            temp[1] = word[i+1];
            temp[2] = '\0';             // End the temp array to create C string with null at the end
            // Get the points for the two-letter combinations
            int twoLetterPoints = getLetterPoints(temp);
            if (twoLetterPoints > 0) {                    // If the two-letter combination has a positive point value, add it to the total points
                points += twoLetterPoints;                  
                i++;                          // Skip the next letter since it's part of the two-letter combination
                continue;
            }
        }
        temp[0] = word[i];                  // If we didn't find a valid two-letter combination, check for a single-letter card
        temp[1] = '\0';                     // End the temp array 
        points += getLetterPoints(temp);    // Get the points for the single-letter card and add it to the total points
    }

    return points;        // Return the total points calculated for the input word
}

// Prints the valid words and their corresponding point value
void printResults(char *words[], int length, char **cards, int cardsLength) {
    // Iterate through the array of words
    for (int i = 0; i < length; i++) {
        // Check if the length of the current word is greater than 2
        if (strlen(words[i]) > 2) {            
            int points = wordPoints(words[i], cards, cardsLength, strlen(words[i]));    // Calculate the points for the current word using the wordPoints function
            printf("\t%d: %s | %d\n", i, words[i], points);         // Print the index, word, and points of the current word 
        }
    }
}
...
void processGuess(char *cardString) {
    ...
    printResults(matchedWords, matchedCount, cards, cardCount);
